...
There are lots of applications of machine learning, and the most popular problem in practice is __binary classification__.
...
* isS (isSignal), isB (isBackground) &mdash; how many observations really belong to this class
* asS (asSignal), asB (asBackground) &mdash; how many observations were classified as signal (background)
* isSasB (isSignalasBackground) &mdash; how many signal observations were erroneously classified as background.
  isSasS, isBasB, isBasS are defined in the same way.


One can hardly misunderstand what each of the introduced numbers means.
...
Another way to define parameters is True/False Positives/Negatives:
 
``` 
TP = isSasS, FP = isBasS, TN = isBasB, FN = isSasB  
```

But it is too easy (at least for me) to mix up FP and FN, so I prefer to avoid this notion.
 
## Continuous predictions. ROC curve 

As it is clear now, there are different ways to measure quality of binary predictions.
The bad thing is that people frequently start by comparing those.

Meanwhile, the output of a classifier is __real-valued, not binary__.

What's wrong with using binary predictions?


1. to estimate quality, one needs to select a threshold. Usually people forget about this (and use the default value, which is far from the optimal one)
2. binary metrics are frequently unstable and need many samples in validation (statistical variation is comparable to gain obtained by changing parameters),
   this leads to unstable model selection.
3. in many cases, a trained classifier is not used to make decisions, but is needed to reconstruct the probabilities later used in the next stages of the ML pipeline
...
How 'far' are the distributions of output of signal and background. 
...
For each threshold `tpr` and `fpr` are computed (which part of signal/background events passes this threshold).
...
This presentation was prepared with help of Oleg Alenkin.
...
<div class="layout-wrapper">
    ...
    <script src="/scripts/jquery-3.7.1.min.js" charset="utf-8"></script>
...
However, the curve is not a number, and the final quality still should be computed according to the real problem (based on the ROC curve).

__accuracy__: assuming that you have $N_s$ signal objects and $N_b$ background objects, 
the expected number of correctly guessed samples is $N_s \times TPR + N_b \times (1 - FPR)$, to maximize the number 
of correctly predicted objects, you should select a threshold, which corresponds to the maximum of this value on the ROC.
(As you can check, at this point ROC should be tangent to line $N_s \times TPR - N_b \times FPR = const$).

__weighted accuracy__: another example (which is common in practice). 
Imagine that for each correct guess about a new object you are paid.
But the cost for correctly guessing signal and background are different: $c_s, c_b$. 
To maximize the income, the threshold for the decision rule should correspond to a point on the ROC curve, where
 $$ N_s \times TPR \times c_s + N_b \times (1 - FPR) \times c_b $$ is maximal.

One of such examples is email spam filtering:
cost for incorrect classification of ham letter is n times higher than cost for making wrong prediction for spam letter.
 
In particle physics more complicated expressions are usually taken as a final measure of classification quality.
These correspond to hypothesis testing (Punzi metric, median significance), but the idea is still the same: 
...
* [yhat about ROC](https://blog.yhathq.com/posts/roc-curves.html)
* [python implementation of ROC](https://scikit-learn.org/stable/auto_examples/model_selection/plot_roc.html)
