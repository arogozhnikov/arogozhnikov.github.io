...
I'm introducing a new approach to reweighting of samples. To begin with, let me describe what it is about and why it is needed.

Reweighting is a general procedure, but its major use-case for particle physics is to modify the output of Monte-Carlo (__MC__) simulation 
to reduce disagreement with real data (__RD__) collected at collider.

Why do we need any simulation? When looking for rare decays, we need to train a classifier to discriminate the searched 
particle/decay from everything else. For this purpose we use a simulation of those events we expect to find in real data. 
The classifier is trained using this data. However, the simulation is always imperfect, because it contains some approximations.

Thus we need to calibrate the simulation.
 
For this purpose we take some well known physical process, for which there is achievable real data (real data is usually [sPlot-ted]({% post_url 2015-10-07-splot %}) to eliminate the contribution of background).
...
That's how this looks in the simplest case:

![distributions before reweighting and after reweighting](/images/reweighter/1-reweighting.png)

Pay attention that the only thing we change is the weights of the MC distribution. 
...
It is very simple: you split the features you are reweighting into bins. 
...
What can be said about this method? This method is very fast and intuitive, however it has strong limitations:

1. It handles one (two if enough data) variables, and is almost never able to reweight more variables.
...
Why am I interested in comparing in the context of this post?
...
Checking only 1-dimensional projections (which is what is usually done) is not enough - that's what you can see on the plots below. 
These distributions have identical 1d projections, but the distributions themselves are very different:
...
The major problem here is that we are trying to answer the wrong question. 
Two-sample tests say whether two samples of data you have were generated from the same distribution.

We know that the answer is __NO__: simulated and real data are different. 

We know these two samples are different even in the nature of appearance.
The question we are interested in is __whether the classification model used in the analysis discriminates simulation results and real data__.

If not, this is good news: different numbers computed on Monte-Carlo of the signal channel are reliable. 
You can, in your computation, substitute real data of the signal channel (which you can't even get) with simulated data.
  
  
So, we need to check that the classifier is not able to make a difference between RD and MC. 
This is simple: train a classifier to discriminate RD and MC and look at the ROC curve built for test data. 
...
This gives you an idea of the impact of the disagreement between distributions on the final classification quality (classification quality is checked on real background vs simulated signal).
In the case that this ROC curve is very high, you're in trouble.

## Caveats of reweighting  
 
Let's move back to reweighting with bins. 
Even with this simple reweighting algorithm there are some caveats, which may not be very obvious.
...
After looking at the result (which is the second row), I see that now these distributions match perfectly, 
so probably I should be very happy with this result.

But I am not, because I am interested in __applying the reweighting rule to a different channel__. 
So first I test the rule on a holdout &mdash; a different chunk of data from the same distribution (this is the third row), 
and I see those strange artifacts. They appeared because __in some bins there were too few events__ and the ratio used in reweighting became very unstable. 
...
The amount of data needed to provide a stable reweighting rule grows exponentially as well.
...
But these regions shall be found in some more intelligent way than just 'split each variable into n parts'. 

For this purpose we use decision trees. 
Recall that a decision tree, by checking simple conditions like $\text{feature}_i > \text{threshold}$, 
splits the feature space into bins, each one associated with a leaf of the tree.
...
If the summand is high, reweighting in the bin is needed.
...
By checking all possible thresholds, I find out that the optimal one in the sense of $\chi^2$ is right in the middle. 
To the left we need to decrease the weight of the blue distribution, while to the right we need to increase it. Both found bins are good, and the threshold seems to be close to optimal.

Since we are unable to solve the global optimization problem and find the optimal structure of the tree, 
we optimize greedily, each time splitting some region into a couple of new regions (as it is usually done in decision trees). 

## Gradient Boosted Reweighter

Let's move on. The gradient boosted reweighter consists of many such trees. 
...
(thus final weight, being an exponent, is obtained as a product of contributions from different trees). 
...
In this example I reweighted 11 variables of Monte-Carlo with the BDT reweighter (I also call it GB reweighter). 
To the left you can see the original state; there is some obvious disagreement.

To the right you can see the result of reweighting, and I shall admit this picture is quite boring.
...
Here is a comparison of Kolmogorov-Smirnov distances for histogram reweighting and GB. 
Histogram reweighting was applied to the last two variables, which have high disagreement. 
At the same time GB reweighted all variables, and you can clearly see it in the results. It even managed to get comparable results in the last two variables. 
...
As I stressed earlier, while these one-dimensional checks are necessary, they are not sufficient. 
...
* The BDT Reweighter repeatedly builds trees with few but large leaves
* It is applicable to data of high dimensionality
* And when applied to the same data, it requires less data compared to the histogram method.
* On the other hand it is slow (since this is equivalent to training a GBDT), but, given that analysis lasts for months, 
  I think we can afford to spend 5 minutes training a reweighter.
