...
The picture above shows what this distribution should look like, 
...
And vice versa in the second bin.
...
In other words, let's say we had 8000 $e^{-}$ + 2000 $e^{+}$ in the first bin and 8000 $e^{+}$ + 2000 $e^{-}$ 
...
But when there are more than two bins, the solution is not unique. There are many appropriate combinations of sWeights — which one to choose (like in the example below with 3 bins)?
 
![proportions inside each bin](/images/splot/5-3bins.png)

But things are more complex in practice. We do not have bins, but continuous distributions (which can be treated as many bins). 
...
### Of course we don't have labels for which events are signal and which are background.
...
But we know a priori that the background is distributed as an exponential distribution and signal as a gaussian (more complex models can be met in practice, but the idea is the same).
...
Make sure you see the connection with the previous plot):
...
As you can see, there are also negative sWeights, which are needed to compensate the contributions of the other class 
...
Reconstructed and splotted variables are correlated, but when you consider only signal they are independent!

To demonstrate why this is important, let's use sweights to reconstruct the mass (obviously the mass is correlated with the mass):

![distributions](/images/splot/12-splot-to-reconstruct-mass.png)

# Derivation of sWeights

Now, after we have seen how this works, let's derive the formula for sWeights.
...
The mathematical expectation should be obviously equal to $p_s N_s$, where $N_s$ is the total amount of signal events available from fitting.

Let's also introduce a random variable $1_{x \in bin}$, which is 1 iff event $x$ lies in the selected bin. 

The __estimate for the number of signal events in a bin__ is equal to:
$$X = \sum_x sw_s(x) \; 1_{x \in bin},$$ where $sw_s(x)$ are sPlot weights and are yet to be determined.
...
This way we can guarantee that the average contribution of background is 0 
...
Having solved this linear system, we get the needed coefficients (as those in the paper)

NB. There is a minor difference between the $V$ matrix I use and the $V$ matrix in the paper. 
<br />


## Minimization of variation

The previous part allows one to get the correct result. But there is still no reason for linearity given — we just *assumed this*.
...
A bit complex, isn't it? Instead of optimizing such a complex expression (which is individual for each bin), let's minimize its __uniform upper estimate__
...
Let's write the Lagrangian of the optimization problem:
...
The main assumption we used here is that the distribution inside each bin is absolutely identical.

In other words, we stated that there is no correlation between the index of the bin and the reconstructed variable. 
...
2. the only information used is the probabilities taken from a fit over the variable. In fact, any probability estimates fit well.
3. the source of probabilities should be statistically independent from the reconstructed variable (for each class!).
4. the mixture may contain more than 2 classes (this is supported by `hep_ml.splot` as well)
...
An [updated version](https://indico.cern.ch/event/940874/contributions/3953530/) is publicly available.
