...
There are traditions which most developers of ML libraries are trying to follow, 
...
Treat this as a "collection of awful advice". 
...
1. __Never care about whether other libraries exist.__
   Why should you actually worry that there are other libraries of machine learning and how many of them are already on GitHub?
   Definitely, the one written by you will be many times better.
...
3. __Introduce your own data format.__ Any successful library has its own format.
   ...
   (additionally, I would recommend not checking the number of columns at prediction time; this would allow the user to make mistakes that are hard to find).


4. __Don't use random seed.__ At least don't do it in the way users expect. There are various interesting options:

   1. simply never set seed, so debugging would be almost impossible. If algorithm didn't converge 10 times in a row, that's because of bad luck (or karma).
...
5. __Write in C++ or CUDA__. You heard there are other languages,
   ...
   While reading your code, another programmer will not be able to get what information is used by each method.

   
6. __Write lots of logs to the output!__
   Remember the starting point? There is nothing else the user should care about, other than your library?
   ...
   It should always return code 0 (whatever happens) and print errors in stdout (not stderr!).
...
9. Write __your own map-reduce system.__
   ...
   Setting up a distributed environment with the same collection of libraries is very simple.
    
   Finally, there is always some user who wants a 'distributed' option
...
If I convinced you that there is no need for new libraries, but you still wish to do something useful in ML,
there are projects which you can help with.

Firstly, those are:

* theano (it looks like there are ~3-4 active contributors)
* numpy (parallel operations in numpy are very much needed there, as well as optimization of existing code)
* scikit-learn (which in some cases lacks multi-threading)

Even a minor contribution to those projects is of more help, because lots of software relies on them.
Writing wrappers for projects you're using is also a good idea.

If you still feel that you want to write a new library, spend much time with scikit-learn first.
Understand its interface, how Bagging/AdaBoost work as meta-estimators.
How grid-search for such meta-algorithms is done.
Probably, read the sources.

Then start writing. Leave the fewest dependencies possible and write simple code (using numpy or so). 
If it is required, you'll rewrite it to optimize for speed. Make sure your library is installed via a package manager
