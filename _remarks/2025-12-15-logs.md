...
From this perspective, a "good" log should contain everything — there is rarely a clear convention for what should *not* be logged (aside from obvious things like credentials).
...
A common suggestion is to tune logging filters, assuming one can identify which parts generate redundant logs.
...
Debates about what should be `TRACE/DEBUG/INFO/WARN/ERROR/FATAL` are as old as log levels themselves.
...
This sounds actionable, but it isn’t &mdash; logging systems lack essential context to make a call on important/not important.
...
If the code was just committed, shouldn't the same error be more severe?
...
What if the person running the program is also the one providing the input &mdash; would it be helpful to assign the highest level to the message?
...
Last, but not least... I've never seen admins actually being awakened by logging. Either they are on shift if the system is critical, or there is a special handle not tied to logging. 
...
Instead, an entry in the log should appear only if the code took more time than expected, and this entry should contain all relevant information.
...
One log entry is written upon entrance to the context manager, and one more on exit.

Analyzing logs means matching these relationships, and Eliot provides tooling to visualize logs as a tree.
It also does not use log levels: everything is logged, and "filtering" happens later via observing the tree.
...
Inputs and outputs shall be tracked — but not logs.
...
**Sidenotes:** I don't discuss here observability solutions that have an explicit (questionable) goal of tracking everything (like OpenTelemetry). 
These systems are designed with causality tracking &mdash; hardly avoidable due to the sheer amount of information.
I also should mention that in languages without GC keeping arbitrary context requires additional hassle.
...
Update: Boris Tane argues for a [similar](https://loggingsucks.com/) (but more explicit) approach with "wide events".
